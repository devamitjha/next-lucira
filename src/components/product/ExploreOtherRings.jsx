...
export default function ExploreOtherRings() {
  ...
  return (
    <section className="w-full mt-15 px-4 md:px-16 bg-white overflow-hidden">
      <div className="w-full">
        <h2 className="text-3xl font-bold text-center mb-8">Explore Other Rings</h2>

        {/* Toggle Tabs */}
        <div className="flex justify-center mb-8">
          <div className="flex bg-[#F8F8F8] p-1 rounded-md border border-gray-100">
            <button 
              className={`px-10 py-2.5 rounded-md text-sm font-bold transition-all ${activeTab === 'shape' ? 'bg-black text-white shadow-lg' : 'text-gray-500 hover:text-black'}`}
              onClick={() => handleTabChange('shape')}
            >
              By Shape
            </button>
            <button 
              className={`px-10 py-2.5 rounded-md text-sm font-bold transition-all ${activeTab === 'style' ? 'bg-black text-white shadow-lg' : 'text-gray-500 hover:text-black'}`}
        ...
        <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-12">
          {SHAPES.map((shape) => (
            <button
              ...
              className={`flex items-center gap-2.5 px-6 py-2 rounded-full border transition-all ${
            ...
            >
              ...
              <span className="text-sm font-bold">{shape.label}</span>
            </button>
          ))}
        </div>
